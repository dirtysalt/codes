...
def build_tree(nums, s, e):
    if s == e:
        return Tree((s, e), nums[s], nums[s])

    m = (s + e) // 2
    l = build_tree(nums, s, m)
    r = build_tree(nums, m + 1, e)
    max_v = max(l.max_value, r.max_value)
    min_v = min(l.min_value, r.min_value)
    t = Tree((s, e), max_v, min_v)
    t.left = l
    t.right = r
    return t


def query_tree(t: Tree, fm, to, v, ctx):
    if t is None:
        return 0
    (s, e) = t.se
    if to < s or fm > e:
    ...
    if fm > m:
        ...
        a = query_tree(t.left, fm, min(to, m), v, ctx)

    if to <= m:
        b = 0
    else:
        b = query_tree(t.right, max(s, fm), to, v, ctx)

    return a + b


# note(yan): 不知道这种区间树是否正确足够高效. 一个优化点是找到这个值后面的一个点p, 那么p往后的数就可以直接累计过来
# 但是感觉这题想复杂了，如果逆序遍历，每个值都插入二叉树，然后对二叉树查询的话应该是很快的.
class Solution:
    def reversePairs(self, nums: List[int]) -> int:
        ...
        for i in reversed(range(n - 1)):
            ...
            if last[i] != -1:
                ...
                res = his[p]
                ctx['d'] += 1
                res += query_tree(t, j, p, max_exp, ctx)
            else:
                res = query_tree(t, j, n - 1, max_exp, ctx)
            # print(i, res)
            his[i] = res
            ans += res
        print('ctx = {}, native = {}'.format(ctx, self.native(nums)))
        return ans
