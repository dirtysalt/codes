...
std::vector<uint32_t> GenRandomInts(int N) {
    ...
    for(int i=0;i<N;i++) {
        int r = rand() % (1 << 30);      // Returns a pseudo-random integer between 0 and RAND_MAX.
        ans[i] = (uint32_t)r;
    }
    return ans;
}
...
void Bench(std::vector<uint32_t> values, int level) {
    ...
    if (level == 0) {
        ...
        FORI(k,times) {
            ans += avx512_vpopcnt(data, values.size());
        }
    }
    t.stop();
    auto total = t.elapsedMilliseconds();
    std::cout << "[" << name <<"] N = " << N << ", took: " << total << "ms, avg " << (total * 1e3) / N << "ns/N, ans = " << ans << "\n";
}
