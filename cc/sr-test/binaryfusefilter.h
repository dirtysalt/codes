...
static inline binary_hashes_t binary_fuse8_hash_batch(uint64_t hash, const binary_fuse8_t* filter) {
...
static inline uint32_t binary_fuse8_hash(int index, uint64_t hash, const binary_fuse8_t* filter) {
...
static inline bool binary_fuse8_contain(uint64_t key, const binary_fuse8_t* filter) {
    ...
    f ^= filter->Fingerprints[hashes.h0] ^ filter->Fingerprints[hashes.h1] ^ filter->Fingerprints[hashes.h2];
    return f == 0;
}

static inline uint32_t binary_fuse_calculate_segment_length(uint32_t arity, uint32_t size) {
...
static inline double binary_fuse_calculate_size_factor(uint32_t arity, uint32_t size) {
    if (arity == 3) {
...
static inline bool binary_fuse8_allocate(uint32_t size, binary_fuse8_t* filter) {
    ...
    uint32_t initSegmentCount = (capacity + filter->SegmentLength - 1) / filter->SegmentLength - (arity - 1);
    filter->ArrayLength = (initSegmentCount + arity - 1) * filter->SegmentLength;
    filter->SegmentCount = (filter->ArrayLength + filter->SegmentLength - 1) / filter->SegmentLength;
    if (filter->SegmentCount <= arity - 1) {
        filter->SegmentCount = 1;
    } else {
        filter->SegmentCount = filter->SegmentCount - (arity - 1);
    }
    filter->ArrayLength = (filter->SegmentCount + arity - 1) * filter->SegmentLength;
...
inline bool binary_fuse8_populate(const uint64_t* keys, uint32_t size, binary_fuse8_t* filter) {
    ...
    if ((alone == NULL) || (t2count == NULL) || (reverseH == NULL) || (t2hash == NULL) || (reverseOrder == NULL) ||
        (startPos == NULL)) {
        free(alone);
        free(t2count);
        free(reverseH);
    ...
    for (int loop = 0; true; ++loop) {
        ...
        for (uint32_t i = 0; i < size; i++) {
            ...
            if ((t2hash[h0] & t2hash[h1] & t2hash[h2]) == 0) {
                if (((t2hash[h0] == 0) && (t2count[h0] == 8)) || ((t2hash[h1] == 0) && (t2count[h1] == 8)) ||
    ...
    for (uint32_t i = size - 1; i < size; i--) {
        ...
        filter->Fingerprints[h012[found]] =
                xor2 ^ filter->Fingerprints[h012[found + 1]] ^ filter->Fingerprints[h012[found + 2]];
    }
    free(alone);
    free(t2count);
    free(reverseH);
...
static inline binary_hashes_t binary_fuse16_hash_batch(uint64_t hash, const binary_fuse16_t* filter) {
    uint64_t hi = binary_fuse_mulhi(hash, filter->SegmentCountLength);
    binary_hashes_t ans;
    ans.h0 = (uint32_t)hi;
    ans.h1 = ans.h0 + filter->SegmentLength;
...
static inline uint32_t binary_fuse16_hash(int index, uint64_t hash, const binary_fuse16_t* filter) {
    uint64_t h = binary_fuse_mulhi(hash, filter->SegmentCountLength);
    h += index * filter->SegmentLength;
    // keep the lower 36 bits
    uint64_t hh = hash & ((1UL << 36) - 1);
...
// If there are many duplicated keys and you do not want to remove them, you can first
// sort your input, the algorithm will then work adequately.
inline bool binary_fuse16_populate(const uint64_t* keys, uint32_t size, binary_fuse16_t* filter) {
    uint64_t rng_counter = 0x726b2b9d438b9d4d;
    filter->Seed = binary_fuse_rng_splitmix64(&rng_counter);
    uint64_t* reverseOrder = (uint64_t*)calloc((size + 1), sizeof(uint64_t));
    uint32_t capacity = filter->ArrayLength;
