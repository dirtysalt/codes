...
typedef struct xor8_s {
    ...
    uint8_t* fingerprints; // after xor8_allocate, will point to 3*blockLength values
} xor8_t;
...
static inline bool xor_init_buffer(xor_setbuffer_t* buffer, size_t size) {
    ...
    buffer->buffer = (xor_keyindex_t*)malloc(buffer->slotcount * buffer->slotsize * sizeof(xor_keyindex_t));
...
static inline void xor_buffered_increment_counter(uint32_t index, uint64_t hash, xor_setbuffer_t* buffer,
                                                  xor_xorset_t* sets) {
...
static inline void xor_make_buffer_current(xor_setbuffer_t* buffer, xor_xorset_t* sets, uint32_t index,
...
static inline void xor_buffered_decrement_counter(uint32_t index, uint64_t hash, xor_setbuffer_t* buffer,
...
static inline void xor_flush_decrement_buffer(xor_setbuffer_t* buffer, xor_xorset_t* sets, xor_keyindex_t* Q,
                                              size_t* Qsize) {
    size_t qsize = *Qsize;
    for (uint32_t slot = 0; slot < buffer->slotcount; slot++) {
...
static inline uint32_t xor_flushone_decrement_buffer(xor_setbuffer_t* buffer, xor_xorset_t* sets, xor_keyindex_t* Q,
                                                     size_t* Qsize) {
    uint32_t bestslot = 0;
...
inline bool xor8_buffered_populate(const uint64_t* keys, uint32_t size, xor8_t* filter) {
    ...
    while (true) {
        ...
        for (size_t i = 0; i < size; i++) {
            ...
            xor_buffered_increment_counter(hs.h1, hs.h, &buffer1, sets1);
            xor_buffered_increment_counter(hs.h2, hs.h, &buffer2, sets2);
        }
        ...
        while (Q0size + Q1size + Q2size > 0) {
            while (Q0size > 0) {
                ...
                if (sets0[index].count == 0) continue; // not actually possible after the initial scan.
                //sets0[index].count = 0;
                ...
                stack_size++;
                xor_buffered_decrement_counter(h1, hash, &buffer1, sets1, Q1, &Q1size);
                xor_buffered_decrement_counter(h2, hash, &buffer2, sets2, Q2, &Q2size);
            }
            if (Q1size == 0) xor_flushone_decrement_buffer(&buffer1, sets1, Q1, &Q1size);
            ...
            while (Q1size > 0) {
                ...
                if (sets1[index].count == 0) continue;
                //sets1[index].count = 0;
                uint64_t hash = keyindex.hash;
            ...
            if (Q2size == 0) xor_flushone_decrement_buffer(&buffer2, sets2, Q2, &Q2size);
            while (Q2size > 0) {
                xor_keyindex_t keyindex = Q2[--Q2size];
                size_t index = keyindex.index;
                xor_make_buffer_current(&buffer2, sets2, index, Q2, &Q2size);
                if (sets2[index].count == 0) continue;
            ...
            if (Q0size == 0) xor_flushone_decrement_buffer(&buffer0, sets0, Q0, &Q0size);
