...
datatype geom_exp =
	 ...
	 | Var of string
     | Shift of real * real * geom_exp (* shift dx dy exp *)
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(dx, dy, e1) =>
        (case eval_prog(e1, env) of
          ...
          | _ => raise BadProgram("shift on bad type")
        )
...
fun preprocess_prog (e) =
    case e of
        ...
        if (real_close(x1, x2) andalso real_close(y1, y2)) then
            Point(x1, y1)
        else if (not (real_close(x1, x2)) andalso x1 > x2) orelse (real_close(x1, x2) andalso y1 > y2) then
      ...
      | Shift(dx, dy, e) =>
        Shift(dx, dy, preprocess_prog(e))
      | _ => e

(*
LineSegment(0.1,0.3,1.0,3.0)
LineSegment(0.0,0.0,1.0,1.0)
LineSegment(1.00000999,1.0,1.0,2.0)
*)

(* val it0 = preprocess_prog(LineSegment(0.1,0.3,1.0,3.0)) *)
(* val it1 = preprocess_prog(LineSegment(1.0,1.0,0.0,0.0)) *)
(* val it2 = preprocess_prog(LineSegment(1.00000999,1.0,1.0,2.0)) *)
