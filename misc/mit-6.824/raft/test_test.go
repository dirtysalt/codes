...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("disconnect X%d", leader1)
	...
	DPrintf("connect X%d", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	DPrintf("disconnect X%d, X%d", leader2, (leader2+1)%servers)
	...
	DPrintf("connect X%d", (leader2+1)%servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	DPrintf("connect X%d", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
...
//
func TestFigure82XC(t *testing.T) {
	servers := 5
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()
...
func TestFigure8Unreliable2YC(t *testing.T) {
	servers := 5
	cfg := make_config(t, servers, true)
	defer cfg.cleanup()
