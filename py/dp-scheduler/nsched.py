...
from task import RunJobTask,RunCallableTask
...
def main(argv):
    ...
    #logger=getLogger('nsched',logfile)
    ...
    # register siganl handler. ignore it.
    # it's a little bit diffcult to stop http server in another thread.
    def handler(sig, frame):
        signal.signal(signal.SIGINT, signal.SIG_IGN) # ignore next signal.
    ...
    # http server in another thread.
    t=Thread(target=http_run, args=(http,))
    t.start()

    # coord in the main thread.
    coord.start()
    coord.run()
