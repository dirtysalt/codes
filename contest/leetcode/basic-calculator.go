...
func NewStack(size int) *Stack {
	return &Stack{st : make([]int, size), off:0, size:size}
}

func (self *Stack)Dump() {
	fmt.Printf("stack = %v", self.st[:self.off])
}
...
func reduce(vst *Stack, ost *Stack) {
	for ; !ost.IsEmpty() ; {
		v := vst.Pop()
		v2 := vst.Pop()
		if v2 == '(' {
		...
		// fmt.Printf("reduce v = %v, v2 = %v, op = %c, res = %v\n", v, v2, op, res)
		vst.Push(res)
	}
}


func unwind(vst *Stack) {
	v := vst.Pop()
	v2 := vst.Pop()
	if v2 != '(' {
		panic("unwind state error")
	}
	vst.Push(v)
}

func calculate(s string) int {
	vst := NewStack(len(s) + 5)
	ost := NewStack(len(s))
	...
	fmt.Printf("op_cnt = %v, vl_cnt = %v, lb_cnt = %v, rb_cnt = %v\n", op_cnt, vl_cnt,
				lb_cnt, rb_cnt)
	vst.Dump()
	unwind(vst)
	return vst.Pop()
